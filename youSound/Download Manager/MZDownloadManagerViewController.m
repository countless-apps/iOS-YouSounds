...
@interface MZDownloadManagerViewController () <NSURLSessionDelegate, UIActionSheetDelegate>
{
    ...
    __weak IBOutlet UIButton *btnLeftNavigation;
...
- (void)addDownloadTask:(NSString *)fileName fileURL:(NSString *)fileURL
{
    ...
    if([arrURL containsObject:fileURL]){
        return ;
    }
    NSURL *url = [NSURL URLWithString:fileURL];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    ...
    NSString* filePath = [documentsPath stringByAppendingPathComponent:[downloadInfo objectForKey:kMZDownloadKeyFileName]];
    BOOL fileExists = [[NSFileManager defaultManager] fileExistsAtPath:filePath];
    if(fileExists)
    {
        [downloadingArray removeLastObject];
        [MZUtility showAlertViewWithTitle:kAlertTitle msg:@"Allready downloaded"];
    }
    else
    {
...
- (MZDownloadingCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MZDownloadingCell *cell = [bgDownloadTableView dequeueReusableCellWithIdentifier:cellIdentifier];
    
    if (cell == nil)
    {
        NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"MZDownloadingCell" owner:self options:nil];
...
- (void)updateCell:(MZDownloadingCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.lblDetails.textColor = LiteColor;
    cell.lblTitle.textColor = DarkColor;
